...
messagesRouter
  ...
  .get(async (req, res) => {
    try {
      const messages = await Entrie.findAll({ order: [['id', 'DESC']] });
      res.json(messages);
    } catch (error) {
      console.log(error);
      res
...
console.log('POST------------------------------------', req.body)
    try {
      ...
      const newMessage = await Entrie.create({
        title,
        body,
        img: filename,
        userId: res.locals?.user?.id || 1,
      });
...
messagesRouter.get('/my', verifyAccessToken, async (req, res) => {
  try {
    const messages = await Entrie.findAll({ where: { userId: res.locals.user.id } });
    res.json(messages);
  } catch (error) {
    console.log(error);
...
messagesRouter
  ...
  .patch(verifyAccessToken, checkMessageOwner, async (req, res) => {
    try {
      const { title, body } = req.body;
      const message = await Entrie.findByPk(req.params.messageId);
...
messagesRouter
  ...
  .patch(verifyAccessToken, checkMessageOwner, upload.single('img'), async (req, res) => {
    try {
      const oldMessage = await Entrie.findByPk(req.params.messageId);
      if (req.file) {
